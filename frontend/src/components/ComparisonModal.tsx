...
export default function ComparisonModal({ products, onRemove, onClose, allProducts, onSelectProduct }: ComparisonModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/28 p-4 backdrop-blur-md">
      ...
      <div className="relative z-10 flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-[32px] border border-stone-900/10 bg-[linear-gradient(180deg,rgba(255,250,240,0.96),rgba(248,243,232,0.96))] shadow-[0_30px_100px_rgba(112,82,48,0.75)]">
        ...
        <div className="overflow-y-auto p-6 md:p-8">
          {products.length === 0 ? (
            <div className="rounded-[28px] border border-stone-900/10 bg-white/65 p-16 text-center">
            ...
            <div className="space-y-6">
              <div className="grid gap-6 lg:grid-cols-2">
                {[productA, productB].map((product, index) => product ? (
                  <div key={product.id} className="rounded-[28px] border border-stone-900/10 bg-white/65 p-5">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <button onClick={() => onRemove(product)} className="rounded-full border border-stone-900/10 bg-white/65 px-3 py-2 text-xs font-semibold text-editorial-text/70 transition hover:border-red-700/25 hover:bg-red-700/10 hover:text-red-700">
                        Remove
                      </button>
                    </div>

                    <div className="mt-5 rounded-[24px] border border-stone-900/10 bg-stone-100/65 p-4">
                      <div className="space-y-3 text-sm text-editorial-text/68">
                        ...
                        <div className="flex justify-between gap-3"><span>Brand</span><span className="font-semibold text-editorial-text">{product.brand || "Lamania"}</span></div>
                  ...
                  <div key={`placeholder-${index}`} className="rounded-[28px] border border-dashed border-white/12 bg-white/[0.02] p-8 text-center">
                    ...
                    {unusedProducts.length > 0 && (
                      <select onChange={(e) => { const found = allProducts.find((p) => p.id === e.target.value); if (found) onSelectProduct(found); }} defaultValue="" className="mt-5 w-full rounded-2xl border border-stone-900/10 bg-stone-100/80 px-4 py-3 text-sm text-editorial-text outline-none">
              ...
              {productA && productB && (
                <div className="rounded-[28px] border border-stone-900/10 bg-white/65 p-6">
                  <div className="flex flex-col gap-4 rounded-[24px] border border-amber-700/15 bg-[linear-gradient(180deg,rgba(185,135,70,0.12),rgba(185,135,70,0.04))] p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                    ...
                    <button onClick={handleGenerateAIReport} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-semibold text-stone-950 transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-50">
                  ...
                  {aiReport && (
                    <div className="mt-5 rounded-[24px] border border-stone-900/10 bg-stone-100/80 p-5 text-sm leading-7 text-editorial-text/74">
